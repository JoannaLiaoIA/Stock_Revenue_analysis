...
## 更新內容
* 2026.01.08 建立基本雛形。
* 2026.01.10 更新 break-even price 計算方式、放到 github pages。
* 2026.01.12 修正「計算損益金額和簡單報酬率」的篩選條件，改成大於無風險利率才顯示。

## 背景
* 質押股票：台積電（2330）
* 標的股票：台積電（2330）
* 無風險利率：以[台銀一年期定存利率](https://rate.bot.com.tw/twd?Lang=zh-TW)為判斷標準
...
```{R cal return and rate of return , echo = TRUE}
...
df_result <- expand.grid(
  ...
  filter(Return >= (risk_free_rate * 100))   # 只顯示有賺錢的組合
```

### 損益計算結果 {.tabset .tabset-pills}
選擇買進價格查看不同賣出價格下的損益計算結果
```{R cal return on different price buying, results = 'asis'}
...
### 計算結果 {.tabset .tabset-pills .tabset-fade}
